...
bool 
PlaceTask::querySceneGraph(const std::string& node_name, const std::string& relationship_type, const std::string& attribute_name, std::vector<std::string>& related_nodes, std::vector<std::string>& attributes) 
{
...
bool 
PlaceTask::init(const TaskParameters& parameters)
{
   ...
   ***************************************************/
  {
    ...
    if(querySceneGraph(parameters.place_surface_, "", "subframe_names", related_nodes, place_subframes) == false)
    {
      ROS_ERROR("Failed to query scene graph for support surface");
      return 1;
    }
    {
    ...
    auto fallback_container = std::make_unique<Fallbacks>("Place Fallbacks");
      ...
      *****************************************************/
        {
          ...
          p.header.frame_id = parameters.place_surface_ + "/" + subframe;  // collision_object.header.frame_id;  // object_reference_frame was used before
          p.pose.orientation.w = 1; 
          p.pose.position.z = getHeightOffsetForSurface(parameters.object_name_, parameters.place_surface_, parameters.place_surface_offset_);
