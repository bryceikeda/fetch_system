...
bool PourTask::init(const TaskParameters &parameters)
{
  ...
  }
  {
    ...
    .... *               Allow collision (object support)   *
       ***************************************************/
      {
        ...
        stage->allowCollisions({parameters.object_name_}, "table1", true);
        grasp->insert(std::move(stage));
      }
    ...
    .... *               Forbid collision (object support)  *
       ***************************************************/
      {
        ...
        stage->allowCollisions({parameters.object_name_}, "table1", false);
        grasp->insert(std::move(stage));
      }
  ...
  ***************************************************/
...

  {
    ...
    wrapper->setMaxIKSolutions(8);
    wrapper->setIKFrame(parameters.grasp_frame_transforms_.find("horizontal_grasp_frame_transform")->second, parameters.hand_frame_);
