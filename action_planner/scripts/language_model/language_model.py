...
class LanguageModel:
    def __init__(self):
        ...
        self.translation_lm = SentenceTransformer(self.translation_lm_id).to(self.device)
        self.hyperparams = get_hyperparameters(OPENAI_KEY=self.OPENAI_KEY)

        self.example_task_embeddings = self.translation_lm.encode(example_tasks, batch_size=512, convert_to_tensor=True, device=self.device)
    ...
    def query_language_model(self, query):
        ...
        if query["mode"] == "setup trigger":
            ...
            inst_AP_variables.prompt = get_prompt(inst_AP_variables, None, "trigger explain") # construct prompt for explaination

            samples, log_probs = self.generator(inst_AP_variables.prompt, self.hyperparams.sampling_params, self.hyperparams.max_tokens, self.hyperparams.stop) # query Planning LM for single-step action candidates

            inst_AP_variables.explanation = samples[np.argmax(log_probs)]
            inst_AP_variables.prompt = get_prompt(inst_AP_variables, None, "trigger") # construct prompt

            samples, log_probs = self.generator(inst_AP_variables.prompt, self.hyperparams.sampling_params, self.hyperparams.max_tokens, self.hyperparams.stop)
          
            trigger_str = samples[np.argmax(log_probs)].translate(str.maketrans('', '', string.punctuation))
        ...
        if query["mode"] == "generate action plan":
            ...
            for action in inst_AP_variables.action_list:
                if "<object>" in action:
                    for obj in inst_AP_variables.query_obj_names:
                        inst_AP_variables.all_actions.append(action.replace("<object>", obj))
                elif "<surface>" in action:
                    for surface in inst_AP_variables.query_surface_names:
                        inst_AP_variables.all_actions.append(action.replace("<surface>", surface))
                else:
                    inst_AP_variables.all_actions.append(action)
                inst_AP_variables.action_list_embds = self.translation_lm.encode(inst_AP_variables.all_actions, batch_size=512, convert_to_tensor=True, device=self.device)

            inst_AP_variables.prompt = get_prompt(inst_AP_variables, None, "explain") # construct prompt for explaination

            # find most relevant example
            example_idx = get_similar_eg(inst_AP_variables.query_task, self.hyperparams, example_tasks, self.example_task_embeddings, self.translation_lm)
            ...
            inst_AP_variables.prompt = get_prompt(inst_AP_variables, example_idx, "action") # construct prompt
            assert generate_action_plan(inst_AP_variables, self.generator, self.translation_lm, self.hyperparams) # generate action plan
